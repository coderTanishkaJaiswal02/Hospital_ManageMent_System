...
import { Trash2, Edit, PlusCircle, Search, Shield, ChevronDown } from "lucide-react";
...
const DrAvailablities = () => {
  ...
  const currentAvailabilities = filteredAvailabilities.slice(indexOfFirstItem, indexOfLastItem);
            ...
            <div className="hidden md:block">
              ...
              <div className="flex flex-col py-6 gap-2 mt-2">
                {currentAvailabilities.length > 0 ? (
                  currentAvailabilities.map((item, index) => (
                    <div
                      key={item.id}
                    ...
                    >
                      ...
                      <div>{item.time?.map((t) => `${t.start} - ${t.end}`).join(", ") || "No time slots"}</div>
              ...
              {formData.time.map((t, i) => (
                <div key={i} className="flex gap-2">
                  ...
                  <input
                    ...
                    required
                  />
                  <button type="button" onClick={() => removeTimeField(i)} className="text-red-500">
                    X
                  </button>
                </div>
              ))}
